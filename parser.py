...
def read_operations(sheet):
    ...
    for line in range(lines["begin"], lines["end"]):
        ...
        qtd = int(
            sheet.cell_value(line, cols["QTD_COMPRA"])
            if op == "COMPRADA"
            else sheet.cell_value(line, cols["QTD_VENDA"])
        )
        ...
        data.append(
            {
                ...
                "data": datetime.datetime(
                    *xlrd.xldate_as_tuple(
                        sheet.cell_value(line, cols["DATA"]), datemode
                    )
                ).strftime("%d/%m/%Y"),
                "qtd": qtd,
                "preço": format_price(
                    sheet.cell_value(line, cols["PRECO_COMPRA"])
                    if op == "COMPRADA"
                    else sheet.cell_value(line, cols["PRECO_VENDA"]),
                    qtd,
                ),
...
def find_earnings_columns(sheet, lines):
    xls_to_cols = {"Cód. Neg.": "TICKER", "Creditado No Mês": "VALOR"}
    cols = dict((el, None) for el in xls_to_cols.values())
...
def print_operations(ops):
    ...
    for op in ops:
        print(
            f"{op['op']}\t{op['qtd']}\t{op['ticker']}\t{op['qtd']}\tR$ {op['preço']}\tExecutada\t{op['data']}"
        )
